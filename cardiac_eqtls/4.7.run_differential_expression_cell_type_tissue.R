...
run_diffexp = function(x, name, metadata, covariates, tissue1, tissue2)
{
	...
	#vars0             = c("sex", "total_reads_norm", "uniquely_mapped_reads_to_canonical_chromsomes", "mitochondrial_reads")# original
	vars0             = c("sex", "total_reads_norm")# remove mitochondrial reads as covariate
	vars1             = colnames(covariates)[grepl("cibersort" , colnames(covariates)) == TRUE]
	tissues           = c('ipsc_cvpc', 'heart', 'arteria')
	diffexp           = as.data.frame(rbindlist(lapply(tissues, function(tissue)
	{
		message(paste(name, tissue))
		this_covariates = covariates[covariates[,tissue] == 1,]
		this = x[,rownames(this_covariates)]
		out = as.data.frame(rbindlist(lapply(rownames(this), function(gene_id)
		{
			as.data.frame(rbindlist(lapply(vars1, function(var1)
			{
				run_diffexp_lm(gene_id, this, this_covariates, vars0, var1)
	...
	fwrite(diffexp, paste("pipeline/4.1.differential_expression", paste("diffexp_cell_tissue", name, "txt", sep = "."), sep = "/"), sep = "\t", col.names = TRUE, row.names = FALSE)
	
	return(diffexp)
}

diffexp1 = run_diffexp(gene_tpm, "gene_tpm"   , metadata, covariates)
diffexp2 = run_diffexp(isof_tpm, "isoform_tpm", metadata, covariates)
diffexp3 = run_diffexp(isof_use, "isoform_use", metadata, covariates)

diffexp = rbind(diffexp1, diffexp2, diffexp3)

fwrite(diffexp, paste("pipeline/4.1.differential_expression", paste("diffexp_cell_tissue", "txt", sep = "."), sep = "/"), sep = "\t", col.names = TRUE, row.names = FALSE)
